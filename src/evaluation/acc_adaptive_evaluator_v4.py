"""
ACC-Inspired Adaptive Evaluator (V4)
...
class ACCAdaptiveEvaluator:
    ...
    def extract_features(self, question: str) -> torch.Tensor:
        ...
        concat_hidden = torch.cat([hidden_states[f"layer_{l}"] for l in self.layers])
        
        # 2. Apply PCA compression
        compressed = self.pca.transform(concat_hidden.numpy().reshape(1, -1))
        compressed_tensor = torch.tensor(compressed, dtype=torch.float32).squeeze(0)
        
        # 3. Compute dynamic features
        dynamic_features = self._compute_dynamic_features(hidden_states)
        
        # 4. Compute early branching entropy

"""
ACC-Inspired Probe Training (V4.4)

V4.4 increases weight to 50x (from V4.2's 10x) for positive utility samples.

Key changes from V4.2:
- Weighted MSE loss for utility (50x weight on positive samples)
- VERY strong emphasis on minority class
- Aggressive push for positive predictions
...
5. **WEIGHTED loss (50x) for utility prediction** ← VERY STRONG!
...
def train_acc_probe(
    ...
    print(f"  With 50x weighting, positive samples contribute ~{(all_train_utils > 0).sum() * 50 / (len(all_train_utils) + (all_train_utils > 0).sum() * 49) * 100:.1f}% to loss")
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [Train]", leave=False):
            ...
            # V4.4: Weighted utility loss (50x weight for positive samples)
            loss_utility = weighted_utility_loss(utility_pred, utility_labels, weight_positive=50.0)

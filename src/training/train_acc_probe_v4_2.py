"""
ACC-Inspired Probe Training (V4.2)

V4.2 adds WEIGHTED LOSS to handle severe class imbalance in utility labels.

Key changes from V4.1:
- Weighted MSE loss for utility (10x weight on positive samples)
- Addresses the "predict negative for everything" problem
- Should enable positive utility predictions (20-30% of samples)

Trains a compact, value-aware probe with:
1. Dynamic features (9 dims) + early entropy (2 dims) = 11 dims
2. Multi-task heads (wrong, conflict, utility)
3. Value-aware routing (utility gain, not just difficulty)
4. Focal loss for wrong detection
5. **WEIGHTED loss for utility prediction** ← NEW!

Key innovation: Predicts whether metacognition is WORTH IT, not just whether baseline is wrong.
"""
...
class ACCProbe(nn.Module):
    ...
    def forward(self, features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        utility_score = torch.tanh(self.utility_head(x)).squeeze(-1)  # [-1, 1]
...
def train_acc_probe(
    ...
    # V4.2: Create weighted MSE loss function
    # Computes weights dynamically per batch based on utility labels
    def weighted_utility_loss(pred, target, weight_positive=10.0):
        """Weighted MSE loss with higher weight for positive utility samples."""
        weights = torch.ones_like(target)
        positive_mask = target > 0
        weights[positive_mask] = weight_positive
    ...
    print(f"  Zero (-0.5 to 0): {((all_train_utils > -0.5) & (all_train_utils <= 0)).mean()*100:.1f}%")
    print(f"  Negative (<-0.5): {(all_train_utils < -0.5).mean()*100:.1f}%")
    print(f"  Mean: {all_train_utils.mean():.3f}")
    print(f"  With 10x weighting, positive samples contribute ~{(all_train_utils > 0).sum() * 10 / (len(all_train_utils) + (all_train_utils > 0).sum() * 9) * 100:.1f}% to loss")
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [Train]", leave=False):
            ...
            # V4.2: Weighted utility loss (10x weight for positive samples)
            loss_utility = weighted_utility_loss(utility_pred, utility_labels, weight_positive=10.0)

"""
ACC-Inspired Probe Training (V4.3)

V4.3 increases weight to 20x (from V4.2's 10x) for positive utility samples.

Key changes from V4.2:
- Weighted MSE loss for utility (20x weight on positive samples)
- Stronger emphasis on minority class
- Should push more predictions into positive range
...
5. **WEIGHTED loss (20x) for utility prediction** ← STRONGER!
...
def train_acc_probe(
    ...
    print(f"  With 20x weighting, positive samples contribute ~{(all_train_utils > 0).sum() * 20 / (len(all_train_utils) + (all_train_utils > 0).sum() * 19) * 100:.1f}% to loss")
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [Train]", leave=False):
            ...
            # V4.3: Weighted utility loss (20x weight for positive samples)
            loss_utility = weighted_utility_loss(utility_pred, utility_labels, weight_positive=20.0)

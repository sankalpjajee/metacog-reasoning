"""
ACC-Inspired Probe Training (V4)

Trains a compact, value-aware probe with:
1. Compressed input features (~270 dims, not 16,384)
...
class ACCProbe(nn.Module):
    ...
    Input: ~270 dims (compressed hidden + dynamic + entropy)
    ...
    def __init__(
        self,
        input_dim: int = 267,  # 256 compressed + 9 dynamic + 2 entropy
        hidden_dim: int = 128,
        dropout: float = 0.1,
    ):
        super().__init__()
